...
class SaveCase(unittest.TestCase):
    def setUp(self):
        ...
        self.s.ingest([dict(message(1), context_source_refs=['shared/20260901@original'])], now=0)
    ...
    def test_save_is_separate_frozen_and_no_drift_patch_needed(self):
        ...
        manifest = json.loads((self.repo / 'continuity/saves' / (pending['save_id'] + '.json')).read_text())
        self.assertEqual(manifest['journal_files'], ['journal/20260912.md'])
        self.assertEqual(manifest['upstream_refs'], ['shared/20260901@original'])
        self.assertNotIn('source_ids', (self.repo / 'journal/20260912.md').read_text())
