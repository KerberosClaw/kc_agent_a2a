...
A2A 現在提供共同 `recap.json`，保留同場兩隻各自署名的摘要；Party 經受眾審查後可引用。公開 Poke repo 並未由此次發布一起升級，若要讓主動戳也引用，須由該端 adapter 另行接入並驗證。生活日記與各自 journal 的選用來源契約、隱私限制與啟用流程見[共享脈絡](context.md)。
